<script lang="ts">
	...
	const uvLevels = [
		...
		{ label: 'Extreme', min: 11, max: 15, color: 'from-purple-600 to-purple-900' }
	];
...
<div class="flex flex-col items-center w-full max-w-md mx-auto py-4 relative">
	...
	<div class="relative w-full h-2 flex rounded-md overflow-hidden bg-gradient-to-r">
		{#each uvLevels as level, i}
			<div class="flex-1 h-full bg-gradient-to-r {level.color}"></div>
		{/each}

		<!-- UV Indicator (Overlapping the scale) -->
		<div
			class="absolute top-1/2 left-0 z-50 size-2 bg-white border border-gray-500 transition-transform"
			style="left: {getUVPosition(uvIndex)}; transform: translate(-50%, -50%)"
		></div>
	</div>
</div>
