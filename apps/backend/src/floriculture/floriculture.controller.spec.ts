...
describe('FloricultureController', () => {
  ...
  describe('createNotification', () => {
    it('validates owning user and delegates', async () => {
      ...
      const res = await controller.createNotification('5', { title: 'T', message: 'M' }, req);
      expect(res).toEqual({ done: true });
      expect(userService.createNotification).toHaveBeenCalledWith('5', expect.anything());
    });

    it('blocks mismatched sub with Unauthorized', async () => {
      const req = { user: { userId: 666 } } as any;
      await expect(controller.createNotification('100', { title: 'X', message: 'Y' }, req)).rejects.toThrow(UnauthorizedException);
    });
  });

  describe('create & update data handling', () => {
    it('sanitizes invalid dates and parses schema on add', async () => {
      const badPayload = { flower_name: 'Rose', planting_date: 'Invalid Date' } as any;
      mockService.create.mockResolvedValue({ id: 1 });

      await controller.create(badPayload);
      expect(floricultureSchema.partial().parse).toHaveBeenCalledWith(expect.objectContaining({
        planting_date: null,
      }));
