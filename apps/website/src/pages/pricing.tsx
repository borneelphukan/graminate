...
const Pricing = () => {
  ...
  const [currentPlanFromDb, setCurrentPlanFromDb] = useState<string>("FREE");

  useEffect(() => {
    // Check for token in URL (after login redirect) or localStorage
  ...
  useEffect(() => {
    if (userToken && userId) {
      ...
      .then(data => {
        if (data.plan) {
          setCurrentPlanFromDb(data.plan);
        }
      })
  ...
  const handleSubscribe = async (planType: "BASIC" | "PRO") => {
    if (!userToken || !userId) {
      const result = await Swal.fire({
        ...
        text: "Please login or register to subscribe to a plan.",
        icon: "info",
      ...
      if (result.isConfirmed) {
        window.location.href = `http://localhost:3000/login?callbackUrl=${encodeURIComponent(window.location.href)}`;
      }
      return;
    }
    ...
    try {
      ...
      const options = {
        key: process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID || "rzp_test_RPSdEutFlDapRe",
        amount: order.amount,
        ...
        handler: async (response: any) => {
          ...
          try {
            ...
            Swal.fire({
              ...
              text: `Welcome to Graminate ${planType.charAt(0) + planType.slice(1).toLowerCase()}! Your plan has been updated.`,
              icon: "success",
              confirmButtonColor: "#10b981",
            });
            
            setCurrentPlanFromDb(planType);
          } catch (error) {
  ...
  const handleCardClick = async (planId: string) => {
    ...
    if (targetLevel < currentLevel) {
      ...
      if (result.isConfirmed) {
        try {
          const res = await fetch(`http://localhost:3001/api/user/${userId}/schedule-downgrade`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
              "Authorization": `Bearer ${userToken}`
            },
            body: JSON.stringify({ plan: normalizedId })
          });
          const data = await res.json();
          
          Swal.fire({
            title: "Downgrade Scheduled",
            text: data.data?.message || "Your plan will be updated at the end of the billing cycle.",
            icon: "success",
            confirmButtonColor: "#10b981",
          });
        } catch (error: any) {
          console.error("Downgrade error:", error);
          Swal.fire("Error", "Failed to schedule downgrade. Please try again.", "error");
        }
      }
      return;
    }
