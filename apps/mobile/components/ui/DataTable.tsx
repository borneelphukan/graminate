...
DataTable.Header = ({ children, style }: any) => <View className="flex-row bg-gray-50 dark:bg-[#121212] border-b border-gray-200 dark:border-gray-800 py-2" style={style}>{children}</View>;

DataTable.Title = ({ children, numeric, style }: any) => (
...
DataTable.Row = ({ children, style, onPress }: any) => {
...
DataTable.Cell = ({ children, numeric, style }: any) => (
  <View className={`flex-1 px-2 justify-center ${numeric ? 'items-end' : 'items-start'}`} style={style}>
    {typeof children === 'string' ? <RNText className="text-sm text-gray-800 dark:text-gray-200">{children}</RNText> : children}
  </View>
);

DataTable.Pagination = ({ page, numberOfPages, label, onPageChange, showFastPaginationControls, style }: any) => (
