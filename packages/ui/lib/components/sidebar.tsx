...
export const Sidebar = ({
  ...
  return (
    <div
      className={`sidebar-container fixed inset-y-0 left-0 bg-gray-800 text-gray-300 shadow-xl transform transition-transform duration-300 ease-in-out z-30 flex flex-col ${isOpen ? "translate-x-0" : "max-lg:-translate-x-full"
    ...
    >
        ...
        {items.map((item) => {
          ...
          return (
            <div key={item.label} className={`relative ${isCollapsed ? "px-1" : "px-3"}`}>
              <div
                className={`flex items-center rounded-lg cursor-pointer group transition-colors duration-200 ${isCollapsed ? "p-2 justify-center" : "p-3"
                  } ${isActive
              ...
              >
                {typeof item.icon === "string" ? (
                  ...
                  <div
                    className={`h-5 w-5 flex-shrink-0 flex items-center justify-center text-current ${isCollapsed ? "" : "mr-3"
                      }`}
                  >
                ...
                {!isCollapsed && (
                  <>
                    ...
                    {hasSubItems && (
                      <Icon
                        type="chevron_right"
                        className={`h-4 w-4 transition-transform duration-200 ${isSectionOpen ? "rotate-90" : ""
                ...
                <div className="mt-1 ml-5 pl-3 border-l border-gray-600 space-y-1">
                  {item.subItems?.map((sub) => {
                    ...
                    return (
                      <div
                        key={sub.label}
                        className={`text-sm py-2 px-4 rounded-md cursor-pointer transition-colors duration-150 ${isSubActive
      ...
      <div className="mt-auto bg-gray-800 p-3 border-t border-gray-700">
        <button
          className={`w-full flex items-center p-3 rounded-lg text-dark dark:text-light hover:bg-gray-700 hover:text-gray-300 transition-colors duration-200 ${isCollapsed ? "justify-center" : "justify-end"
            }`}
