    ...
    if (Array.isArray(items)) {
      return items.map((item) => (
        <li
          ...
          role="option"
          tabIndex={0}
          className={`px-4 py-2 text-sm transition-colors duration-150 ${disabledItems.includes(item)
  ...
  return (
    <div className={`relative ${containerClasses}`} ref={dropdownRef}>
      {label && (
        <label
          htmlFor={dropdownId}
          className={`block mb-1 text-sm font-medium ${variant === "small" ? "text-dark dark:text-light" : "text-dark dark:text-light"
            }`}
        >
          {label}
        </label>
      ...
      <button
        id={dropdownId}
        type="button"
        className={`w-full flex items-center justify-between transition-all duration-200 border ${variant === "small" ? "p-2.5 text-sm" : "px-4 py-2 text-sm"
      ...
      >
        ...
        <Icon
          ...
          className={`size-5 ml-2 transition-transform duration-200 ${isOpen ? "rotate-180" : ""
